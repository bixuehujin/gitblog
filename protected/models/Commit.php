...
class Commit extends CActiveRecord {
	...
	public function relations() {
		return array(
			'userSettings'=>array(self::HAS_ONE,
				'UserSetting',
				array('uid'=>'uid')),
		);
	}
	 ...
	 *
	 * @param object $commit object contains commit information fetched 
	 * from database.
	 * @throws Exception 
	 */
	protected function parseSingleCommit($commit) {
		...
		$commitInfo = $github->commits()->show($commit->userSettings->github,
				$commit->userSettings->repository,
				$commit->commit_id);
		
		foreach($commitInfo['files'] as $file) {
			$raw = $github->blobs()->show($commit->userSettings->github, 
					$commit->userSettings->repository,
					$file['sha']);
			...
			$meta = $parser->meta;
			if (isset($parser->meta['status']) && 
					$meta['status'] == 'published') {
				
				$timestamp = DateTime::createFromFormat('Y-m-d\TH:i:s\Z',
						$commitInfo['commit']['author']['date'])->getTimestamp();
				
				$postModel = Post::model();
				$post = $postModel->findByPath($file['filename']);
				$isNewPost = $post ? false : true;
				$visibilityMaps = Post::visibilityMaps();
				$visibility = (isset($meta['visibility']) && 
						isset($visibilityMaps[$meta['visibility']])) 
				...
				if (!$post) {
					...
					$post->category_id = isset($meta['category']) 
						? (int)Category::getIdByName($meta['category']) : 0;
				...
				$postRevision = PostRevision::model()
					->findByShaAndPostID($file['sha'], $post->post_id);
				...
				if(isset($meta['category']) && 
						$categoryId = Category::getIdByName($meta['category'])) {
